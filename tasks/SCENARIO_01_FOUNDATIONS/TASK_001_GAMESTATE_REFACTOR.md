...
3. On click → read `currentMap` from profile → `SceneManager.navigateTo(mapUrl)`
4. Target scene's `onEnter` restores position + world state
...
- Pixi.js is **not directly involved** in state persistence. Keep `GameState` / `SaveManager` as pure JS modules with zero Pixi dependency.
- On save load, the scene's `onEnter(params)` callback receives restored data and uses it to rebuild the Pixi scene graph (recreate sprites at saved positions, restore destroyed entities).
- Example: `forest-main.js` `onEnter()` reads `maps.forest.choppedTrees` and removes those tree sprites from `LAYERS.ground`.
