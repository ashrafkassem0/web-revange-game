...
- Audio is independent of Canvas 2D rendering. Keep `AudioManager` as a standalone module (extend `sounds.js`).
- Scene init (e.g. `forest-main.js`) calls `AudioManager.playMusic('forest_theme')`, `AudioManager.playAmbient('forest_ambient')`.
- On scene exit / before `navigateTo`, call `AudioManager.crossfadeMusic('forest_theme', 'city_theme', 2000)` or stop ambient loops.
- Volume UI lives in HTML settings overlay (same pattern as forest HUD / city modals), not drawn on canvas.
